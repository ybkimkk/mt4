                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <table id="basic-datatable" class="table dt-responsive nowrap table-hover" width="100%">
                                            <thead>
                                                <tr>
                                                    <th class="no-sort"><?php echo L('客户');?></th>
                                                    <th class="no-sort"><?php echo L('类型');?></th>
                                                    <th class="no-sort"><?php echo L('内容');?></th>
...
<?php
	...
	$query = $DB->query("select a.*,b.nickname,b.phone,b.email from (select * from `t_message_board` {$where} order by addtime desc LIMIT {$sqlRecordStartIndex},{$pagersize}) a left join `t_member` b on a.m_id = b.id");
	if($DB->numRows($query) <= 0){
		echo '<tr><td colspan="99" class="dataTables_empty" valign="top">' , L('无可用数据') , '</td></tr>';
	}else{
		while($rs = $DB->fetchArray($query)){
			echo '<tr>';
			echo '<td>' , $rs['nickname'] , '<br>' , $rs['email'] , '</td>';
			echo '<td>' , $GuestbookTypeArr[$rs['type']] , '</td>';
			echo '<td>' , FShortStr2($rs['content'],20) , '</td>';
			echo '<td>' , date('Y-m-d H:i:s',$rs['addtime']) , '</td>';
			echo '<td>' , $rs['status'] == 1 ? '<font color="green">' . L('已回复') . '<font>' : '<font color="red">' . L('未回复') . '<font>' , '</td>';
			echo '<td>';
			if($rs['status'] == 1){
				echo '<a href="?clause=showinfo&id=' . $rs['id'] . '" class="btn btn-light btn-sm">' . L('查看') . '</a> ';
			}else{
				echo '<a href="?clause=showinfo&id=' . $rs['id'] . '" class="btn btn-primary btn-sm">' . L('回复') . '</a> ';
			}
