                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    <h4 class="page-title"><?php echo L('赠金报表') , getCurrMt4ServerName();?></h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
...
<?php
$isSearch = 0;
if($SearchGroupName || $SearchQ || $SearchUserType || $SearchReject || $SearchSTime || $SearchETime){
	$isSearch = 1;
}
...
$result = getCapitaList(4, $page, $SearchSTime, $SearchETime, '', $member_where['loginID'], $member_where['allMID'] );
...
if(FGetInt('isdownload') > 0){
	...
	$objPHPExcel->setActiveSheetIndex(0)->setTitle(L('赠金报表'));
	...
	foreach($list as $key=>$rs){
		...
		$i++;$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit($i . $j,  $rs['credit'],PHPExcel_Cell_DataType::TYPE_STRING);
		$i++;$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit($i . $j,  $rs['countCredit'],PHPExcel_Cell_DataType::TYPE_STRING);
...
echo '&nbsp; &nbsp;' , L('总赠金') , '：';
if($totaldata['totalcredit'] < 0){       
    echo '<font color="red">$' , round($totaldata['totalcredit'],2) , '</font>';
}else{
	echo '<font color="green">';
	echo '$' , round($totaldata['totalcredit'],2);
	echo '</font>';
}
echo '&nbsp; &nbsp;' , L('赠金笔数') , '：' , $totaldata['totalcountCredit'];
?> 
</div>
                                        ...
                                        <table id="basic-datatable" class="table dt-responsive nowrap table-hover" width="100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="no-sort"><?php echo L('赠金笔数');?></th>
                                                </tr>
                                            </thead>
                                            <tbody>
<?php
		...
		if(count($list) <= 0){
			...
			foreach($list as $key=>$rs){
				...
				echo '<td>';
				if($mtserver_ver == 5){
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?czv=mt5&report_type=1&LOGIN=' , $rs['loginid'] , '&CMD=7&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="text-' , $rs['credit'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['credit'] , '</label>';
				}else{
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?LOGIN=' , $rs['loginid'] , '&CMD=7&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="text-' , $rs['credit'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['credit'] , '</label>';
				}
				echo '</td>';
				echo '<td>';
				if($mtserver_ver == 5){
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?czv=mt5&report_type=1&LOGIN=' , $rs['loginid'] , '&CMD=7&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="number-text">' , $rs['countCredit'] , '</label>';
				}else{
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?LOGIN=' , $rs['loginid'] , '&CMD=7&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="number-text">' , $rs['countCredit'] , '</label>';
				}
				echo '</td>';
				echo '</tr>';
			}
