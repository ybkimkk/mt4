                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    <h4 class="page-title"><?php echo L('转账报表') , getCurrMt4ServerName();?></h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
...
$result = getCapitaList(3, $page, $SearchSTime, $SearchETime, '', $member_where['loginID'], $member_where['allMID'] );
...
if(FGetInt('isdownload') > 0){
	...
	$objPHPExcel->setActiveSheetIndex(0)->setTitle(L('转账报表'));
...
echo '&nbsp; &nbsp;' , L('转账笔数') , '：' , $totaldata['totalincount'];
?> 
</div>
                                        ...
                                        <table id="basic-datatable" class="table dt-responsive nowrap table-hover" width="100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="no-sort"><?php echo L('转出金额') , '/' , L('转出笔数');?></th>
                                                    <th class="no-sort"><?php echo L('转入金额') , '/' , L('转入笔数');?></th>
                                                </tr>
                                            </thead>
                                            <tbody>
<?php
		...
		if(count($list) <= 0){
			...
			foreach($list as $key=>$rs){
				...
				echo '</td>';
				echo '<td>';
				echo '<label onclick="javascript:window.location.href=\'?clause=mtdetail&id=' , $rs['loginid'] , '&type=0&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="text-' , $rs['outnumber'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['outnumber'] , '</label>';
				echo '/';
				echo '<label class="number-text">' , $rs['outcount'] , '</label>';
				echo '</td>';
				echo '<td>';
				echo '<label onclick="javascript:window.location.href=\'?clause=mtdetail&id=' , $rs['loginid'] , '&type=1&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="text-' , $rs['innumber'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['innumber'] , '</label>';
				echo '/';
