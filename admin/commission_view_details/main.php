...
//}
	...
	//"select 'vtotal',sum(comm.AMOUNT) as TAMOUNT,count(comm.ID) as TNUM from t_sale_commission comm {$join} where {$where}";exit;	
	$totalAmount = $DB->getField2Arr("select 'vtotal',sum(AMOUNT) as TAMOUNT,count(ID) as TNUM from t_sale_commission {$where}");

	$totalAmount2 = "select sum(VOLUME) AS TVOLUME FROM (select VOLUME,TICKET,COMM_MEMBER_ID,LOGIN,SERVER_ID,BALANCE_TIME,BALANCE_ID,STATUS FROM t_sale_commission GROUP BY TICKET) AS comm {$where}";
...
//echo "select comm.*,  member.nickname, nickname, member.email,member.realname realname," . $DRAdmin['mt4dbname'] . ".mt4_trades.SYMBOL," . $DRAdmin['mt4dbname'] . ".mt4_trades.CLOSE_TIME from t_sale_commission comm {$join} where {$where} order by {$field} {$order} {$pageSql}";exit;
...
if($DRAdmin['ver'] == 5){
	//echo "select a.*,b.nickname, b.email,b.realname,c.Symbol as SYMBOL,c.Time as CLOSE_TIME from (select * from t_sale_commission {$where} order by {$field} {$order} {$pageSql}) a left join t_member b on a.COMM_MEMBER_ID = b.id left join " . $DRAdmin['mt4dbname'] . ".mt5_deals c on a.TICKET = c.PositionID where c.Entry = 1";
	$list = $DB->getDTable("select a.*,b.nickname, b.email,b.realname,c.Symbol as SYMBOL,c.Time as CLOSE_TIME from (select * from t_sale_commission {$where} order by {$field} {$order} {$pageSql}) a left join t_member b on a.COMM_MEMBER_ID = b.id left join " . $DRAdmin['mt4dbname'] . ".mt5_deals c on a.TICKET = c.PositionID where c.Entry = 1 order by {$field} {$order}");
}else{
	$list = $DB->getDTable("select a.*,b.nickname, b.email,b.realname,c.SYMBOL,c.CLOSE_TIME from (select * from t_sale_commission {$where} order by {$field} {$order} {$pageSql}) a left join t_member b on a.COMM_MEMBER_ID = b.id left join " . $DRAdmin['mt4dbname'] . ".mt4_trades c on a.TICKET = c.TICKET order by {$field} {$order}");
}
