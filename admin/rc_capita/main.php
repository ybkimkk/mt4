                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    <h4 class="page-title"><?php echo L('资金报表') , getCurrMt4ServerName();?></h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
...
<?php
$isSearch = 0;
if($SearchGroupName || $SearchQ || $SearchUserType || $SearchReject || $SearchSTime || $SearchETime || $SearchStatus){
	$isSearch = 1;
}
...
$result = getCapitaList('', $page, $SearchSTime, $SearchETime, '', $member_where['loginID'], $member_where['allMID'] );
...
if(FGetInt('isdownload') > 0){
	...
	$objPHPExcel->setActiveSheetIndex(0)->setTitle(L('资金报表'));
	...
	foreach($list as $key=>$rs){
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit($i . $j, $rs['loginid'],PHPExcel_Cell_DataType::TYPE_STRING);
		...
		$i++;$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit($i . $j,  $rs['incount'],PHPExcel_Cell_DataType::TYPE_STRING);
		$i++;$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit($i . $j,  $rs['innumber'],PHPExcel_Cell_DataType::TYPE_STRING);
		
		$i++;$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit($i . $j,  $rs['outcount'],PHPExcel_Cell_DataType::TYPE_STRING);
		$i++;$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit($i . $j,  $rs['outnumber'],PHPExcel_Cell_DataType::TYPE_STRING);
		
		$i++;$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit($i . $j,  $rs['netmoney'],PHPExcel_Cell_DataType::TYPE_STRING);
...
echo L("汇总统计") , '：';
if($DRAdmin['userType'] == 'member'){
	echo '&nbsp; &nbsp; ' , L('代理数') , '：' , $totaldata['agentnumber'];
	echo '&nbsp; &nbsp; ' , L('直客数') , '：' , $totaldata['directnumber'];
	echo '&nbsp; &nbsp; ' , L('员工数') , '：' , $totaldata['membernumber'];
}else if($DRAdmin['userType'] == 'agent'){
	echo '&nbsp; &nbsp; ' , L('代理数') , '：' , $totaldata['agentnumber'];
	echo '&nbsp; &nbsp; ' , L('直客数') , '：' , $totaldata['directnumber'];
}else if($DRAdmin['userType'] == 'direct'){
	echo '&nbsp; &nbsp; ' , L('直客数') , '：' , $totaldata['directnumber'];
}
echo '&nbsp; &nbsp;' , L('总入金') , '：';
if($totaldata['totalinnumber'] < 0){       
    echo '<font color="red">$' , round($totaldata['totalinnumber'],2) , '</font>';
}else{
	echo '<font color="green">';
	echo '$' , round($totaldata['totalinnumber'],2);
	echo '</font>';
}
echo '&nbsp; &nbsp;' , L('入金笔数') , '：' , $totaldata['totalincount'];
echo '&nbsp; &nbsp;' , L('总出金') , '：';
if($totaldata['totaloutnumber'] < 0){            
    echo '<font color="red">$' , round($totaldata['totaloutnumber'],2) , '</font>';
}else{
	echo '<font color="green">';
	echo '$' , round($totaldata['totaloutnumber'],2);
	echo '</font>';
}
echo '&nbsp; &nbsp;' , L('出金笔数') , '：' , $totaldata['totaloutcount'];
echo '&nbsp; &nbsp;' , L('净入金') , '：';
if($totaldata['totalnetmoney'] < 0){        
    echo '<font color="red">$' , round($totaldata['totalnetmoney'],2) , '</font>';
}else{
    echo '<font color="green">';
    echo '$' , round($totaldata['totalnetmoney'],2);
    echo '</font>';
}
?> 
</div>
                                        ...
                                        <table id="basic-datatable" class="table dt-responsive nowrap table-hover" width="100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="no-sort"><?php echo L('入金笔数');?></th>
                                                    <th class="no-sort"><?php echo L('入金金额');?></th>
                                                    <th class="no-sort"><?php echo L('出金笔数');?></th>
                                                    <th class="no-sort"><?php echo L('出金金额');?></th>
                                                    <th class="no-sort"><?php echo L('净入金金额');?></th>
                                                    <th class="no-sort"><?php echo L('账号余额');?></th>
                                                </tr>
                                            </thead>
                                            <tbody>
<?php
		...
		if(count($list) <= 0){
			...
			foreach($list as $key=>$rs){
				...
				echo '<td>';
					echo $rs['nickname'] , '<br>';
					echo L('MT账号') , '：' , $rs['loginid'] , '<br>';
					echo 'MT &nbsp;' , L('名称') , '：' , $rs['loginname'];
				echo '</td>';
				echo '<td>';
					echo $rs['level'] , L('级') , ',';
					if($rs['userType'] == 'agent'){
				...
				echo '</td>';
				echo '<td>';
					echo '<label class="number-text">' , $rs['incount'] , '</label> ';
				echo '</td>';
				echo '<td>';
				if($mtserver_ver == 5){
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?czv=mt5&report_type=1&CMD=6&PROFIT_s=0.01&LOGIN=' , $rs['loginid'] , '&BALANCE=1&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="text-' , $rs['innumber'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['innumber'] , '</label>';
				}else{
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?CMD=6&PROFIT_s=0.01&LOGIN=' , $rs['loginid'] , '&BALANCE=1&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="text-' , $rs['innumber'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['innumber'] , '</label>';
				}
				echo '</td>';
				echo '<td>';
				echo '<label class="number-text">' , $rs['outcount'] , '</label>';
				echo '</td>';
				echo '<td>';
				if($mtserver_ver == 5){
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?czv=mt5&report_type=1&CMD=6&PROFIT_e=-0.01&LOGIN=' , $rs['loginid'] , '&BALANCE=-1&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="text-' , $rs['outnumber'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['outnumber'] , '</label>';
				}else{
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?CMD=6&PROFIT_e=-0.01&LOGIN=' , $rs['loginid'] , '&BALANCE=-1&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="text-' , $rs['outnumber'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['outnumber'] , '</label>';
				}
				echo '</td>';
				echo '<td>';
				if($mtserver_ver == 5){
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?czv=mt5&report_type=1&CMD=6&LOGIN=' , $rs['loginid'] , '&datascope=my&BALANCE=2&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;"  class="text-' , $rs['netmoney'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['netmoney'] , '</label>';
				}else{
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?CMD=6&LOGIN=' , $rs['loginid'] , '&datascope=my&BALANCE=2&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;"  class="text-' , $rs['netmoney'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['netmoney'] , '</label>';
				}
				echo '</td>';
				echo '<td>';
					echo '<label class="text-' , $rs['balance'] > 0 ? 'green' : 'danger' , ' number-text">$' , $rs['balance'] , '</label>';
				echo '</td>';
				echo '</tr>';
			}
		}
