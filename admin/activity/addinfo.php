...
if($Id > 0){	
	$DRInfo = $DB->getDRow("select * from `t_activity_list` where id = '{$Id}' and server_id = '{$DRAdmin['server_id']}'");
	if(!$DRInfo){
		FJS_AB(L('未找到该数据或权限不足'));
	}
	
	$osLangArr = get_lang_otherset_arr('-_activity_-' . $DRInfo['id'],$DRAdmin['server_id']);
}
?>
                                        ...
                                        <form class="form-horizontal" action="?clause=saveinfo&id=<?php echo $Id;?>" method="post" target="iframe_qpost">
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8 mt-1">
                                                	<select name="f_key">
                                                    	<option value=""><?php echo L('活动信息');?></option>
                                                    </select>
                                                </div>
                                            </div>
                                            <?php
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <select class="form-control" name="cid">
                                                        <option value="1"<?php echo $DRInfo['cid'] == 1 ? ' selected' : '';?>><?php echo L('系统活动');?></option>
                                                         <option value="2"<?php echo $DRInfo['cid'] == 2 ? ' selected' : '';?>><?php echo L('滚动活动');?></option>
                                                    </select>
                                                </div>
                                            </div>
											...
											-->
                                            <div class="form-group row">
                                                <label class="col-sm-2"><?php echo L("开始时间");?>：</label>
                                                <div class="col-sm-8">
                                                    <input name="start_time" type="text" class="form-control" value="<?php echo $DRInfo['start_time'] ? date('Y-m-d H:i:s',strtotime($DRInfo['start_time'])) : date('Y-m-d 00:00:00',strtotime('+1 day'));?>" placeholder="<?php echo L('格式参考') , ': ' , date('Y-m-d H:i:s');?>">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label class="col-sm-2"><?php echo L("结束时间");?>：</label>
                                                <div class="col-sm-8">
                                                    <input name="end_time" type="text" class="form-control" value="<?php echo $DRInfo['end_time'] ? date('Y-m-d H:i:s',strtotime($DRInfo['end_time'])) : date('Y-m-d 23:59:59',strtotime('+11 day'));?>" placeholder="<?php echo L('格式参考') , ': ' , date('Y-m-d H:i:s');?>">
                                                </div>
                                            </div>
											<!--
                                            <div class="form-group row">
