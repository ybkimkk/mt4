                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    <h4 class="page-title"><?php echo L('挂单报表') , getCurrMt4ServerName();?></h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
...
if(FGetInt('isdownload') > 0){
	...
	foreach($list as $key=>$rs){
		...
		$i++;$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit($i . $j,  $rs['count'],PHPExcel_Cell_DataType::TYPE_STRING);
...
echo '&nbsp; &nbsp;' , L('总挂单') , '：' , $totaldata['VOLUME'] , L('笔');
?> 
</div>
                                        ...
                                        <table id="basic-datatable" class="table dt-responsive nowrap table-hover" width="100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="no-sort"><?php echo L('挂单量');?></th>
                                                </tr>
                                            </thead>
                                            <tbody>
<?php
		...
		if(count($list) <= 0){
			...
			foreach($list as $key=>$rs){
				...
				echo '<td>';
					...
					echo '<br/>';
					echo '<small>' , L('MT名称') , '：' , $rs['NAME'] , '</small>';
				echo '</td>';
				if($DRAdmin['userType'] != 'direct' || $webConfig['f_directCanRecom'] > 0){
					echo '<td>';
						echo $rs['level'] , L('级') , ',';
				...
				echo '<td>';
				if($mtserver_ver == 5){
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?czv=mt5&LOGIN=' , $rs['LOGIN'] , '&order_type=2,3,4,5,6,7&orderTime_s=' , $SearchSTime , '&orderTime_e=' , $SearchETime , '\'" style="cursor:pointer;" class="number-text">' , $rs['count'] , L('笔') , '</label>';
				}else{
					echo '<label onclick="javascript:window.location.href=\'report_history_trade.php?LOGIN=' , $rs['LOGIN'] , '&CMD=2,3&datascope=my&closetime=2&CLOSE_TIME_s=' , $SearchSTime , '&CLOSE_TIME_e=' , $SearchETime , '\'" style="cursor:pointer;" class="number-text">' , $rs['count'] , L('笔') , '</label>';
				}
				echo '</td>';
				echo '</tr>';
			}
