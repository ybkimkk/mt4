...
<?php
	$osLangArr = array();
	$query = $DB->query("select f_type,f_lang,f_title from t_lang_otherset where f_serverId = '{$DRAdmin['server_id']}' and f_type like '-_news_-%' and f_lang = '{$CurrLangName}'");
	while($rs = $DB->fetchArray($query)){
		$osLangArr[$rs['f_type'].'-'.$rs['f_lang']] = $rs['f_title'];
	}


	$where = "where server_id = '{$DRAdmin['server_id']}' and `status` = 1 and f_key = ''";
	...
	if($DB->numRows($query) <= 0){
		...
		while($rs = $DB->fetchArray($query)){
			...
			echo $osLangArr['-_news_-' . $rs['id'].'-'.$CurrLangName];
			echo '</a>';
			echo '</td>';
