  ...
  private function GetParamUpdate($user)
    {
		...
		if(strlen($user->Rights)){
			$arr[MTProtocolConsts::WEB_PARAM_RIGHTS] = $user->Rights;
		}
		if(strlen($user->Group)){
			$arr[MTProtocolConsts::WEB_PARAM_GROUP] = $user->Group;
		}
		if(strlen($user->Name)){
			$arr[MTProtocolConsts::WEB_PARAM_NAME]  = $user->Name;
		}
		if(strlen($user->Company)){
			$arr[MTProtocolConsts::WEB_PARAM_COMPANY] = $user->Company;
		}
		if(strlen($user->Language)){
			$arr[MTProtocolConsts::WEB_PARAM_LANGUAGE] = $user->Language;
		}
		if(strlen($user->Country)){
			$arr[MTProtocolConsts::WEB_PARAM_COUNTRY] = $user->Country;
		}
		if(strlen($user->City)){
			$arr[MTProtocolConsts::WEB_PARAM_CITY] = $user->City;
		}
		if(strlen($user->State)){
			$arr[MTProtocolConsts::WEB_PARAM_STATE] = $user->State;
		}
		if(strlen($user->ZipCode)){
			$arr[MTProtocolConsts::WEB_PARAM_ZIPCODE] = $user->ZipCode;
		}
		if(strlen($user->Address)){
			$arr[MTProtocolConsts::WEB_PARAM_ADDRESS] = $user->Address;
		}
		if(strlen($user->Phone)){
			$arr[MTProtocolConsts::WEB_PARAM_PHONE] = $user->Phone;
		}
		if(strlen($user->Email)){
			$arr[MTProtocolConsts::WEB_PARAM_EMAIL] = $user->Email;
		}
		if(strlen($user->ID)){
			$arr[MTProtocolConsts::WEB_PARAM_ID] = $user->ID;
		}
		if(strlen($user->Status)){
			$arr[MTProtocolConsts::WEB_PARAM_STATUS] = $user->Status;
		}
		if(strlen($user->Comment)){
			$arr[MTProtocolConsts::WEB_PARAM_COMMENT] = $user->Comment;
		}
		if(strlen($user->Color)){
			$arr[MTProtocolConsts::WEB_PARAM_COLOR] = $user->Color;
		}
		if(strlen($user->PhonePassword)){
			$arr[MTProtocolConsts::WEB_PARAM_PASS_PHONE] = $user->PhonePassword;
		}
		if(strlen($user->Leverage)){
			$arr[MTProtocolConsts::WEB_PARAM_LEVERAGE] = $user->Leverage;
		}
		if(strlen($user->Agent)){
			$arr[MTProtocolConsts::WEB_PARAM_AGENT] = $user->Agent;
		}
