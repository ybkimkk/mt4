...
if($_GET['clause'] == 'topay'){
	...
	$bankinfo = FPostStr('bankinfo');
	if(strlen($bankinfo) <= 0){
		FJS_AB(L('请选择银行'));
	}
	...
	$banknameArr = explode('|',$bankinfo);
	$payInfo['attach'] = base64_encode(json_encode(array('c'=>$banknameArr[0],'n'=>$banknameArr[1])));
	...
	$PAY_BANK_LIST = array("008"=>"MANDIRIATM",
							"013"=>"PERMATAATM",
							"002"=>"EPAYBRI",
							"013"=>"PERMATANETPAY",
							"037"=>"ARTHAGRAHAATM",
	...
	echo '<div class="form-group row">
				<label class="col-sm-2">' , L("银行") , '：</label>
				<div class="col-sm-8">
					<select name="bankinfo" id="bankinfo" class="form-control">';
	foreach($PAY_BANK_LIST as $key=>$val){
