...
if($_GET['clause'] == 'topay'){
	...
	$payInfo = array(
		...
		'customerNo' => $DRAdmin['id'],
		'amount' => $DRInfo['price'],
		'notifyUrl'=> (FIsHttps() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/pay_s2s_' . trim($DRPay['PayCode']) . '.html',
		'timestamp'=>time(),
		'customerName' => $DRAdmin['accountName'],
		'customerAccNo' => $DRAdmin['accountNum'],
	);
...
function http_post_json($url, $jsonStr){
	...
	$html = http_post_json(trim($DRPay['submit_gateway']) . '/gateway/deposit-trans', json_encode($payInfo));
	
	$arr = json_decode($html,true);
	
	//Array ( [code] => 0 [data] => Array ( [accountName] => 张三 [transId] => 202111010000100409 [branchName] => [bankName] => 中国银行 [accountNumber] => 6228100120023003 [transUrl] => https://dev-fms-h5.clippay.com.au/trans/MjAyMTExMDEwMDAwMTAwNDA5 ) [message] => success ) 
	/* 
	{
	  "errorCode" : "1001",
	...
	*/
	...
	if($arr['code'] == '0' && $arr['message'] == 'success'){
		FRedirect($arr['data']['transUrl']);
		exit;
		...
		FCreateErrorPage(array(
			'title'=>L("提示"),
			'content'=>'ERROR：（' . $arr['code'] . '）' . $arr['message'],
			'btnStr'=>L('返回'),
			'url'=>'/',
			'isSuccess'=>0,
			'autoRedirectTime'=>0,
