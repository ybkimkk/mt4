...
if($_GET['clause'] == 'topay'){
	...
	$payInfo = array(
		...
		'callbackurl' => (FIsHttps() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/pay_back_' . trim($DRPay['f_payFolder']) . '_' . trim($DRPay['PayCode']) . '.html',    //支付后，浏览器跳转回来所要到达的页面
		's2surl' => (FIsHttps() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/pay_s2s_' . trim($DRPay['f_payFolder']) . '_' . trim($DRPay['PayCode']) . '.html', //支付成功后，接收 支付服务器通知支付结果情况 的页面
		'bankname'=>'none',
	);
