...
/*if($_GET['clause'] == 'topay'){
	...
	$DRPay = $DB->getDRow("select a.*,b.f_title,b.f_pa,b.f_ers,b.f_fixedEROut,b.f_symbolsER,b.f_erAlgo from (select * from t_pay where `Status` = 1 and server_id = '{$DRAdmin['server_id']}' and Id = '{$DRInfo['pay_id']}') a left join t_pay_currency b on a.f_currencyId = b.id");
	if(!$DRPay){
		FJS_AT(L('未找到支付方式'),'?');
	}
	...
	echo '<input type="hidden" name="confirmsave" value="2">';
	/*
	echo '<div class="form-group row">
			<label class="col-sm-2">' , L("银行账户姓名") , '：</label>
			<div class="col-sm-8">
				<input name="forwordname1" id="forwordname1" type="text" class="form-control" value="' , $bank_name['0']['accountName'] , '" placeholder="银行账户姓名">
			</div>
		</div>';
	echo '<div class="form-group row">
			<label class="col-sm-2">' , L("开户行") , '：</label>
			<div class="col-sm-8">
				<input name="bankname1" id="bankname1" type="text" class="form-control" value="' , $bank_name['0']['bankName'] , '" placeholder="请输入转账银行行号开户行名称">
			</div>
		</div>';
	*/
	echo '<input type="hidden" name="bankname1" value="USDT-TRC20">';
	echo '<div class="form-group row">
			<label class="col-sm-2">USDT-TRC20 ' , L("收款地址") , '：</label>
			<div class="col-sm-8">
				<input name="bankaccount1" id="bankaccount1" type="text" class="form-control" value="" placeholder="">
			</div>
		</div>';
	/*
	echo '<div class="form-group row">
